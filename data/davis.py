...
class TrainDAVIS(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # Randomly sample a video and a frame.
        ...
        masks = (self.to_mask(mask) != 0).long().unsqueeze(0)  # [L=1, H, W]

        # `path` is useful for debugging / bookkeeping (frame file name).
        frame_file = os.path.basename(mask_list[frame_id])
        return {"imgs": imgs, "flows": flows, "masks": masks, "video": video_name, "path": frame_file}
...
class TestDAVIS(torch.utils.data.Dataset):
    ...
    def get_snippet(self, video_name, frame_ids):
        ...
        # Add an explicit channel dimension to match the evaluation code path: [B, L, 1, H, W].
        masks = torch.stack([self.read_mask(os.path.join(mask_path, '{:05d}.png'.format(i))) for i in frame_ids]).unsqueeze(0).unsqueeze(2)
        if self.year == '2016':
            masks = (masks != 0).long()
        files = ['{:05d}.png'.format(i) for i in frame_ids]
        return {'imgs': imgs, 'flows': flows, 'masks': masks, 'files': files, 'video': video_name}
