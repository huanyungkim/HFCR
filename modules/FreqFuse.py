...
def hamming2D(M, N):
    ...
    Returns:
        A 2D Hamming window as a numpy array.
    """
    # 1D Hamming windows for each axis.
    ...
    def _forward(self, hr_feat, lr_feat):
        ...
        if self.semi_conv:
            if self.comp_feat_upsample:
                if self.use_high_pass:
                    # Initial high-pass filter features from hr_feat.
                    ...
                    mask_hr_init = self.kernel_normalizer(mask_hr_hr_feat, self.highpass_kernel,
                                                          hamming=self.hamming_highpass)  # normalize kernels
                    ...
                    # Initial low-pass filter features.
                    ...
                    # Another low-pass branch from lr_feat, then upsample to hr resolution.
                    ...
                    mask_lr = mask_lr_hr_feat + mask_lr_lr_feat  # merge low-pass branches


                    mask_lr_init = self.kernel_normalizer(mask_lr, self.lowpass_kernel,
                                                          hamming=self.hamming_lowpass)
                    # Apply low-pass kernels to lr_feat and upsample to hr resolution.
                    ...
                    mask_hr = mask_hr_hr_feat + mask_hr_lr_feat  # final high-pass features
                else:
...
def compute_similarity(input_tensor, k=3, dilation=1, sim='cos'):
    """
    Computes local similarity between each location and its KxK neighborhood.

    Args:
        input_tensor: [B, C, H, W]
        k: neighborhood window size
        dilation: dilation used by unfolding
        sim: 'cos' for cosine similarity, 'dot' for dot-product similarity

    Returns:
        similarity: [B, K*K-1, H, W] (excluding the center location)
    """
    B, C, H, W = input_tensor.shape
    # Optional padding could be used to handle borders.
    # padded_input = F.pad(input_tensor, (k // 2, k // 2, k // 2, k // 2), mode='constant', value=0)

    # Unfold neighborhood patches: [B, C*K*K, H*W] -> [B, C, K*K, H, W]
    ...
    # Similarity against the center location.
    if sim == 'cos':
    ...
    # Remove the center element -> K*K-1 channels.
    similarity = torch.cat((similarity[:, :k * k // 2], similarity[:, k * k // 2 + 1:]), dim=1)

    # Reshape back to [B, K*K-1, H, W].
...
if __name__ == '__main__':
    ...
    # Sanity check: after fusion, features align at the same spatial resolution and channel count.
    print(f"hr_feat.shape: {hr_feat.shape}")  # high-resolution feature
    print(f"lr_feat.shape: {lr_feat.shape}")  # upsampled low-resolution feature
